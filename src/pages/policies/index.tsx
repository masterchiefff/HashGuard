...
import { 
  ...
  Zap, Clock, Smartphone, ChevronDown, ChevronUp,
  ArrowUpRight
} from "lucide-react";
...
export default function PoliciesPage() {
  ...
  // Plan options with coverage details
  const planOptions = {
    Daily: {
      amount: 1.55, // HBAR (previously 20 KSh)
      coverage: {
        rider: { accidentalDeath: 200000, medicalExpenses: 50000, hospitalCash: 1000 },
        bike: { theft: 0, accidentalDamage: 0, thirdPartyLiability: 100000 }
      }
    },
    Weekly: {
      amount: 7.75, // HBAR (previously 100 KSh)
      coverage: {
        rider: { accidentalDeath: 500000, medicalExpenses: 200000, hospitalCash: 2000 },
        bike: { theft: 150000, accidentalDamage: 50000, thirdPartyLiability: 500000 }
      }
    },
    Monthly: {
      amount: 27.13, // HBAR (previously 350 KSh)
      coverage: {
        rider: { accidentalDeath: 1000000, medicalExpenses: 500000, hospitalCash: 3000 },
        bike: { theft: 300000, accidentalDamage: 150000, thirdPartyLiability: 1000000 }
      }
    }
  };
  ...
  const handlePlanChange = (value: "Daily" | "Weekly" | "Monthly") => {
    setSelectedPlan(value);
    setCoverageDetails(planOptions[value].coverage);
  };
    ...
    try {
      ...
      if (paymentMethod === 'mpesa') {
        response = await axios.post(`${API_BASE_URL}/paypremium`, {
          phone: user.phone,
          amount: planOptions[selectedPlan].amount * 12.9, // Convert to KSh for M-Pesa
          plan: selectedPlan,
          paymentMethod: 'mpesa'
        });
        ...
        response = await axios.post(`${API_BASE_URL}/paypremium`, {
          phone: user.phone,
          amount: planOptions[selectedPlan].amount, // Use HBAR directly
          plan: selectedPlan,
          paymentMethod: 'hbar'
        });
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`font-medium ${
                policies.some(p => p.active) ? "text-green-400" : "text-red-400"
              }`}>
              ...
              <div className="border border-gray-600 rounded-lg p-4">
                ...
                <ul className="space-y-3">
                  <li className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {coverageDetails.bike.theft > 0 
                        ? `${coverageDetails.bike.theft.toLocaleString()} KSh` 
                        : "Not covered"}
                    </span>
                  </li>
                  <li className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {coverageDetails.bike.accidentalDamage > 0 
                        ? `${coverageDetails.bike.accidentalDamage.toLocaleString()} KSh` 
                        : "Not covered"}
                    </span>
                  </li>
                  <li className="flex justify-between">
          ...
          <div className="bg-[#2D3748] rounded-lg p-4">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="font-medium">
                  {selectedPlan} • {planOptions[selectedPlan].amount} KSh
                </p>
              </div>
              ...
              <div className="bg-gray-700/30 rounded-lg p-3">
                ...
                <p className="text-2xl font-bold my-1">
                  {planOptions[selectedPlan].amount} KSh
                </p>
                <p className="text-gray-400 text-sm">
                  {planOptions[selectedPlan].duration === "1 day" ? "Daily" : ""}
                  {planOptions[selectedPlan].duration === "7 days" ? "Weekly" : ""}
                  {planOptions[selectedPlan].duration === "30 days" ? "Monthly" : ""} Renewal
                </p>
              </div>
              ...
              >
                {policies.some(p => p.active) ? "Modify Plan" : "Get Covered"}
              </Button>
        ...
        <div ref={policySectionRef} className="mb-6">
          {paymentStep === "select" && (
            <Card className="bg-[#2D3748] border-none">
              <CardHeader>
                <CardTitle className="text-white">Choose Your Coverage Plan</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div>
                    ...
                    <Select value={selectedPlan} onValueChange={handlePlanChange}>
                      <SelectTrigger className="bg-gray-700 border-none">
                      ...
                      <SelectContent className="bg-gray-700 border-none">
                        <SelectItem value="Daily">
                          <div className="flex justify-between w-full">
                            ...
                            <span className="text-gray-400 text-white">20 KSh</span>
                          </div>
                        </SelectItem>
                        <SelectItem value="Weekly">
                          <div className="flex justify-between w-full">
                            <span className="text-white">Weekly</span>
                            <span className="text-gray-400 text-white">100 KSh</span>
                          </div>
                        </SelectItem>
                        <SelectItem value="Monthly">
                          <div className="flex justify-between w-full">
                            <span className="text-white">Monthly</span>
                            <span className="text-gray-400 text-white">350 KSh</span>
                          </div>
                        </SelectItem>
                      </SelectContent>
                    </Select>
                  ...
                  <div className="border border-gray-600 rounded-lg p-4">
                    ...
                    <ul className="space-y-2 text-sm">
                      <li className="flex items-center text-white">
                        ...
                        {selectedPlan === "Daily" && "Basic accident coverage"}
                        {selectedPlan === "Weekly" && "Full rider + basic bike coverage"}
                        {selectedPlan === "Monthly" && "Comprehensive rider + bike coverage"}
                      </li>
                      <li className="flex items-center text-white">
                        <CheckCircle className="h-4 w-4 mr-2 text-green-500" />
                        {planOptions[selectedPlan].duration} coverage
                      </li>
                      <li className="flex items-center text-white">
                        <CheckCircle className="h-4 w-4 mr-2 text-green-500" />
                        Instant activation
            ...
            <CardContent>
                ...
                {paymentMethod === "mpesa" ? (
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        <span className="font-medium text-white">
                        {(planOptions[selectedPlan].amount / 12.9).toFixed(2)} HBAR
                        </span>
                    </div>
                    {user.walletBalance < (planOptions[selectedPlan].amount / 12.9) && (
                ...
                <div className="bg-gray-700/30 rounded-lg p-3">
                    ...
                    <span className="font-medium text-white">{planOptions[selectedPlan].amount} KSh</span>
                    </div>
                    <div className="flex justify-between mt-1">
                    <span className="text-gray-400">Plan:</span>
                    <span className="font-medium text-white">{selectedPlan}</span>
                    </div>
                    {paymentMethod === "hbar" && (
                    <div className="flex justify-between mt-1">
                        <span className="text-gray-400">HBAR Amount:</span>
                        <span className="font-medium text-white">
                        {(planOptions[selectedPlan].amount / 12.9).toFixed(2)} HBAR
                        </span>
                    </div>
                ...
                <div className="flex gap-2">
                    ...
                    disabled={isLoading || 
                        (paymentMethod === "hbar" && user.walletBalance < (planOptions[selectedPlan].amount / 12.9))
                    }
                    >
          ...
          <CardContent>
              ...
              <div className="overflow-x-auto">
                  ...
                  <thead>
                      ...
                      <th className="pb-3 pl-2 font-medium">Plan</th>
                  ...
                  <tbody>
                      ...
                      <tr key={index} className="border-b border-gray-700 hover:bg-gray-700/30">
                          ...
                          <div className="text-sm text-gray-400">
                              {policy.plan === "Daily" ? "1 day" : ""}
                              {policy.plan === "Weekly" ? "7 days" : ""}
                              {policy.plan === "Monthly" ? "30 days" : ""}
                          </div>
                          </td>
                          <td className="py-4 text-gray-100">
                          {policy.premiumPaid} KSh
                          </td>
                          <td className="py-4">
                          <div className="text-gray-100">
                              {new Date(policy.lastPayment).toLocaleDateString()}
                          </div>
